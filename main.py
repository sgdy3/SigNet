...
def img_preprocess(file_name1,m_lab,f_lab,mod='train',ext_h=820,ext_w=890):
    ...
    if mod=='train':
        m_lab= keras.utils.np_utils.to_categorical(m_lab,50) # 后续使用了categorical_crossentropy，转换为one-hot
    elif mod=='test':
        m_lab=m_lab
    else:
...
def path_extra(user_ord):
    ...
    org_num = 24
    label_dict=dict(zip(user_ord,range(user_ord.shape[0])))  # 原用户标签映射到用户数量range之内，以进行独热编码
    file_path=[]
